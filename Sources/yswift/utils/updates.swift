...
public func decodeUpdateV2(update: Data, YDecoder: (Lib0Decoder) throws -> UpdateDecoder = UpdateDecoderV2.init) throws -> DecodedUpdate {
    var structs: [Struct] = []
    let updateDecoder = try YDecoder(Lib0Decoder(data: update))
    let lazyDecoder = try LazyStructReader(updateDecoder, filterSkips: false)
...
public func encodeStateVectorFromUpdateV2(
    ...
    let updateDecoder = try LazyStructReader(YDecoder(Lib0Decoder(data: update)), filterSkips: false)
...
public func parseUpdateMetaV2(update: Data, YDecoder: (Lib0Decoder) throws -> UpdateDecoder = UpdateDecoderV2.init) throws -> UpdateMeta {
    var from: [Int: Int] = [:]
    var to: [Int: Int] = [:]
...
public func mergeUpdatesV2(
    ...
    let updateDecoders = try updates.map{ try YDecoder(Lib0Decoder(data: $0)) }
...
public func diffUpdateV2(
    ...
    let decoder = try YDecoder(Lib0Decoder(data: update))
...
public func flushLazyStructWriter(lazyWriter: LazyStructWriter) {
    if lazyWriter.written > 0 {
        ...
        lazyWriter.encoder.restEncoder = Lib0Encoder()
